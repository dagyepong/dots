...
local options = {
	...
	tone_mapping = "clip", -- Changed default to "clip" for better compatibility with mpv 0.37+
	...
	if (full and options.tone_mapping ~= "no") or options.tone_mapping == "auto" then
		if properties["video-params"] and properties["video-params"]["primaries"] == "bt.2020" then
			...
			if not tone_mappings[tone_mapping] then
				tone_mapping = "clip" -- Fallback for compatibility
			end
...
local function spawn(time)
	...
	local args = {
		...
		"--vd-lavc-fast",
		"--vd-lavc-threads=" .. (options.hwdec and 1 or 2), -- Optimized: fewer threads if hwdec
		"--hwdec=" .. (options.hwdec and "auto-copy" or "no"), -- Optimized: auto-copy for hw frames
		...
		"--ofopts=update=1",
		"--o=" .. options.thumbnail,
		"--cache=no", -- Added: disable cache for lower memory use
		"--sws-allow-zimg=no",
	}
