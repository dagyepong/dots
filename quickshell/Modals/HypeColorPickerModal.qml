...
HypeModal {
    id: root
    readonly property var log: Log.scoped("HypeColorPickerModal")

    layerNamespace: "hype:color-picker"
    ...
    function pickColorFromScreen() {
        hideInstant();
        Proc.runCommand("hype-color-pick", ["hype", "color", "pick", "--json"], (output, exitCode) => {
            if (exitCode !== 0) {
                log.warn("hype color pick exited with code:", exitCode);
                root.show();
                return;
            }
            try {
                ...
                if (result.hex) {
                    ...
                    log.warn("Failed to parse hype color pick output: missing hex");
                    root.show();
                }
            } catch (e) {
                log.warn("Failed to parse hype color pick JSON:", e);
                root.show();
            }
        }, 0, Proc.noTimeout);
    }
