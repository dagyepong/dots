...
Item {
    ...
    readonly property real activeTitleWidth: root.isCentered ? Math.min(420 * Appearance.effectiveScale, root.centeredWidth * 0.35) : Math.min(560 * Appearance.effectiveScale, root.width * 0.34)
    readonly property real sideHoverPadding: 8 * Appearance.effectiveScale
    readonly property real leftHoverX: Math.max(0, leftCluster.x - sideHoverPadding)
    readonly property real leftHoverWidth: Math.min(root.width - leftHoverX, Math.max(42 * Appearance.effectiveScale, leftCluster.implicitWidth + sideHoverPadding * 2))
    readonly property real centerHoverWidth: Math.min(root.width * 0.26, Math.max(300 * Appearance.effectiveScale, dynamicIsland.width + 260 * Appearance.effectiveScale))
    readonly property real notchExclusionWidth: Math.max(96 * Appearance.effectiveScale, dynamicIsland.pill.width + 48 * Appearance.effectiveScale)
    readonly property real dashboardSideHoverWidth: Math.max(44 * Appearance.effectiveScale, (centerHoverWidth - notchExclusionWidth) / 2)
    readonly property real rightHoverX: Math.max(0, rightCluster.x - sideHoverPadding)
    readonly property real rightHoverWidth: Math.min(root.width - rightHoverX, Math.max(42 * Appearance.effectiveScale, root.width - rightHoverX - privacyIndicator.anchors.rightMargin + sideHoverPadding))
    ...
    function activateHoverPanel(panelName) {
        GlobalStates.openPanelByHover(panelName, root.QsWindow.window.screen);
    }

    function pinAndTogglePanel(panelName) {
        const wasHoverPanel = GlobalStates.hoverPanelName === panelName;
        GlobalStates.activeScreen = root.QsWindow.window.screen;
        GlobalStates.pinPanelOpen();

        if (panelName === "notifications") {
            GlobalStates.notificationCenterOpen = wasHoverPanel ? true : !GlobalStates.notificationCenterOpen;
        } else if (panelName === "quickSettings") {
            GlobalStates.quickSettingsOpen = wasHoverPanel ? true : !GlobalStates.quickSettingsOpen;
        } else if (panelName === "dashboard") {
            GlobalStates.dashboardOpen = wasHoverPanel ? true : !GlobalStates.dashboardOpen;
        }
    }
    ...
    // Left content zone opens Notifications / Weather.
    FocusedScrollMouseArea {
        x: root.leftHoverX
        anchors.top: parent.top
        anchors.bottom: parent.bottom
        width: root.leftHoverWidth
        cursorShape: Qt.PointingHandCursor
        preventStealing: true
        propagateComposedEvents: true
        onEntered: root.activateHoverPanel("notifications")
        onHoveredChanged: if (!hovered) GlobalStates.setHoverTrigger("notifications", false)
        onClicked: {
            root.pinAndTogglePanel("notifications");
        }
    ...
    // Right status zone opens Quick Settings.
    FocusedScrollMouseArea {
        x: root.rightHoverX
        anchors.top: parent.top
        anchors.bottom: parent.bottom
        width: root.rightHoverWidth
        cursorShape: Qt.PointingHandCursor
        preventStealing: true
        propagateComposedEvents: true
        onEntered: root.activateHoverPanel("quickSettings")
        onHoveredChanged: if (!hovered) GlobalStates.setHoverTrigger("quickSettings", false)
        onClicked: {
            root.pinAndTogglePanel("quickSettings");
        }
    ...
    // Time/date zones open Dashboard. The notch itself is excluded so media hover
    // can own the island without the dashboard stealing pointer events.
    MouseArea {
        x: Math.round((root.width / 2) - (root.notchExclusionWidth / 2) - root.dashboardSideHoverWidth)
        anchors.top: parent.top
        anchors.bottom: parent.bottom
        width: root.dashboardSideHoverWidth
        hoverEnabled: true
        cursorShape: Qt.PointingHandCursor
        onEntered: root.activateHoverPanel("dashboard")
        onExited: GlobalStates.setHoverTrigger("dashboard", false)
        onClicked: {
            root.pinAndTogglePanel("dashboard");
        }
    }

    MouseArea {
        x: Math.round((root.width / 2) + (root.notchExclusionWidth / 2))
        anchors.top: parent.top
        anchors.bottom: parent.bottom
        width: root.dashboardSideHoverWidth
        hoverEnabled: true
    ...
    MouseArea {
        ...
        anchors.top: parent.top
        anchors.bottom: parent.bottom
        width: root.notchExclusionWidth
        hoverEnabled: true
        acceptedButtons: Qt.NoButton
        cursorShape: MprisController.activePlayer ? Qt.PointingHandCursor : Qt.ArrowCursor
        onEntered: {
            if (MprisController.activePlayer && (Config.options.media?.enableMediaHover ?? true)) {
    ...
    RowLayout {
        ...
        Item {
            ...
            RowLayout {
                ...
                ActiveWindowTitle {
                    Layout.alignment: Qt.AlignVCenter
                    Layout.preferredWidth: root.activeTitleWidth
                    Layout.maximumWidth: root.activeTitleWidth
                    maxWidth: root.activeTitleWidth
