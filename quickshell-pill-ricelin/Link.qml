 ...
 * `desiredW` and `back()` for the pill's morph and Escape plumbing.
 */
PillSurface {
    id: root
    ...
    readonly property var mainTargets: ["wifi", "bt"]
    readonly property int navCursor: mainTargets.length === 0
        ? -1
        : Math.max(0, Math.min(navIndex, mainTargets.length - 1))
    ...
    function normalizeNav() {
        ...
        if (navIndex < 0)
            navIndex = 0;
        else if (navIndex >= mainTargets.length)
            navIndex = mainTargets.length - 1;
    }

    function mainFocused(key) {
        return subview === "main" && active && mainTargets.length > 0 && mainTargets[navCursor] === key;
    }

    function syncMainFocus() {
        if (subview !== "main" || !active) {
            focusRowItem = null;
            return;
        }
        normalizeNav();
        focusRowItem = mainFocused("wifi") ? netzRow : btRow;
    }

    function moveMain(dir) {
        if (subview !== "main" || mainTargets.length === 0)
            return false;
        normalizeNav();
        navIndex = (navIndex + dir + mainTargets.length) % mainTargets.length;
        syncMainFocus();
        return true;
    }
    ...
    function activateMain() {
        if (subview !== "main" || mainTargets.length === 0)
            return false;
        normalizeNav();
        var target = mainTargets[navCursor];
        if (target === "wifi") {
            subview = "wifi";
        } else if (target === "bt") {
            subview = "bt";
    ...
    function keyPress(key, isAutoRepeat) {
        if (subview === "wifi")
            return wifiPage.keyPress(key, isAutoRepeat);
        if (subview === "bt")
            return btPage.keyPress(key, isAutoRepeat);

        if (key === Qt.Key_Tab)
            return moveMain(1);
        if (key === Qt.Key_Backtab)
            return moveMain(-1);
        if (key === Qt.Key_H || key === Qt.Key_K || key === Qt.Key_Left || key === Qt.Key_Up)
            return moveMain(-1);
        if (key === Qt.Key_L || key === Qt.Key_J || key === Qt.Key_Right || key === Qt.Key_Down)
            return moveMain(1);
    ...
    onSubviewChanged: syncMainFocus()
    ...
    component IconToggleChip: Rectangle {
        id: chip
    ...
    component NotifRow: Rectangle {
        ...
        MouseArea {
            ...
            cursorShape: Qt.PointingHandCursor
            onClicked: {
                Notifs.markEntrySeen(nrow.entry);
    ...
    Item {
        ...
        Column {
            ...
            Rectangle {
                ...
                height: 44 * root.s
                radius: 10 * root.s
                color: netzHover.hovered || root.mainFocused("wifi") ? Theme.frameBg : "transparent"
                border.width: root.mainFocused("wifi") ? 1 : 0
            ...
            Rectangle {
                ...
                height: 44 * root.s
                radius: 10 * root.s
                color: btHover.hovered || root.mainFocused("bt") ? Theme.frameBg : "transparent"
                border.width: root.mainFocused("bt") ? 1 : 0
                border.color: Qt.alpha(Theme.vermLit, 0.78)
                            ...
                            Column {
                                ...
                                Rectangle {
                                    ...
                                    MouseArea {
                                        ...
                                        cursorShape: Qt.PointingHandCursor
                                        onClicked: {
                                            Notifs.markAppSeen(group.modelData.app);
                                            Notifs.toggleExpanded(group.modelData.app);
                                        }
                                    }
