 ...
 *
 * Conditions render as the pill's built-in vector weather icons, keyed off the
 * WMO weather code via `glyphFor`, with `labelFor` giving the short english word.
 */
Singleton {
    id: root
    ...
    /**
     * Maps a WMO weather code to a built-in vector icon name. Clear skies switch
     * between sun and moon so the glance reads day-versus-night at a glance.
     */
    ...
    Component.onCompleted: {
        try {
            ...
            var hasManualCity = Flags.weatherCity && Flags.weatherCity.trim().length > 0;
            if (!hasManualCity && c && typeof c.lat === "number" && typeof c.lon === "number") {
                root.city = c.city || "";
                root.lat = c.lat;
                root.lon = c.lon;
                root.located = true;
    ...
    Process {
        ...
        command: ["curl", "-s", "--max-time", "10",
            ...
            + "&daily=weather_code,temperature_2m_max,relative_humidity_2m_mean&forecast_days=4&past_days=0&timezone=auto"]
        stdout: StdioCollector {
            onStreamFinished: {
                try {
                    var d = JSON.parse(this.text);
