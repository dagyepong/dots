...
Item {
    id: root
    signal closed()
    property bool joinedToBar: false
    readonly property real compactNotificationIslandHeight: 196 * Appearance.effectiveScale
    ...
    Rectangle {
        anchors.fill: parent
        color: root.joinedToBar ? Appearance.colors.colStatusBarSolid : Appearance.colors.colLayer0
        radius: Appearance.rounding.panel
        topLeftRadius: root.joinedToBar ? 0 : Appearance.rounding.panel
        topRightRadius: root.joinedToBar ? 0 : Appearance.rounding.panel
        bottomLeftRadius: Appearance.rounding.panel
        bottomRightRadius: Appearance.rounding.panel
        
        // MD3 Outline Style
        border.width: root.joinedToBar ? 0 : Math.max(1, 1 * Appearance.effectiveScale)
    ...
    ColumnLayout {
        ...
        Rectangle {
            Layout.fillWidth: true
            Layout.preferredHeight: Notifications.list.length > 0
                ? Appearance.sizes.notificationIslandMaxHeight
                : root.compactNotificationIslandHeight
