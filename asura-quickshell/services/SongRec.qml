...
Singleton {
    ...
    readonly property string shellIcon: Directories.home.replace("file://", "") + "/.config/quickshell/nandoroid/assets/icons/asura-shell.svg"
    ...
    function handleRecognition(jsonText) {
        try {
            ...
            Quickshell.execDetached(["notify-send", "-a", "Asura Shell", "-i", root.shellIcon, "--", "Couldn't recognize music", "Perhaps what you're listening to is too niche"])
        }
    }
    ...
    Process {
        ...
        onExited: (exitCode, exitStatus) => {
            if (exitCode === 1) {
                Quickshell.execDetached(["notify-send", "-a", "Asura Shell", "-i", root.shellIcon, "--", "Couldn't recognize music", "Make sure you have songrec installed"])
            }
        }
    }
