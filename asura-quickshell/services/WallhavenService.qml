...
Singleton {
    ...
    function search(query, isMoreLikeThis = false, page = 1) {
        ...
        xhr.open("GET", url);
        xhr.setRequestHeader("User-Agent", "AsuraShell/1.0 (Linux; quickshell)");
            ...
            if (exitCode === 0) {
                ...
                if (apply) {
                    ...
                    Quickshell.execDetached([
                        "sh", "-c", 
                        'curl -L "$1" -o "$2" && notify-send -a "Asura Shell" -i "$3" -- "Wallhaven" "Downloaded: $4"',
                        "sh", url, fullPath, root.shellIcon, fileName
                    ]);
                }
            }
        });
