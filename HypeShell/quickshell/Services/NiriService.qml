...
Singleton {
    ...
    function handleScreenshotCaptured(data) {
        ...
        if (pendingScreenshotPath && data.path === pendingScreenshotPath) {
            const editor = Quickshell.env("HYPE_SCREENSHOT_EDITOR");
            let command;
    ...
    function screenshot() {
        ...
        const path = `${screenshotsDir}/hype-screenshot-${timestamp}.png`;
    ...
    function doGenerateNiriLayoutConfig() {
        ...
        const configContent = hypeWarning + `layout {
        gaps ${gaps}
        ...
        const alttabContent = hypeWarning + `recent-windows {
        highlight {
        ...
        const niriDmsDir = configDir + "/niri/hype";
...
`;
        ...
        let cursorContent = hypeWarning + `cursor {\n`;
