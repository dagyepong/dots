...
func TestGetHYPEVersionInfo_Structure(t *testing.T) {
	// Create a temp directory with a fake HYPE installation
	tempDir := t.TempDir()
	...
	os.MkdirAll(filepath.Join(hypePath, ".git"), 0o755)
	...
	mockFetcher.EXPECT().GetCurrentVersion(hypePath).Return("v0.1.0", nil)
	mockFetcher.EXPECT().GetLatestVersion(hypePath).Return("v0.1.1", nil)

	info, err := GetHYPEVersionInfoWithFetcher(mockFetcher)
	if err != nil {
		t.Fatalf("GetHYPEVersionInfoWithFetcher() failed: %v", err)
	}

	if info == nil {
		t.Fatal("GetHYPEVersionInfoWithFetcher() returned nil")
	}
...
func TestGetHYPEVersionInfo_BranchVersion(t *testing.T) {
	tempDir := t.TempDir()
	hypePath := filepath.Join(tempDir, ".config", "quickshell", "hype")
	os.MkdirAll(hypePath, 0o755)
	os.MkdirAll(filepath.Join(hypePath, ".git"), 0o755)
	...
	mockFetcher := mocks_version.NewMockVersionFetcher(t)
	mockFetcher.EXPECT().GetCurrentVersion(hypePath).Return("master@abc1234", nil)
	mockFetcher.EXPECT().GetLatestVersion(hypePath).Return("master@def5678", nil)
...
func TestGetHYPEVersionInfo_NoUpdate(t *testing.T) {
	tempDir := t.TempDir()
	hypePath := filepath.Join(tempDir, ".config", "quickshell", "hype")
	os.MkdirAll(hypePath, 0o755)
	os.MkdirAll(filepath.Join(hypePath, ".git"), 0o755)
	...
	mockFetcher := mocks_version.NewMockVersionFetcher(t)
	mockFetcher.EXPECT().GetCurrentVersion(hypePath).Return("v0.1.0", nil)
	mockFetcher.EXPECT().GetLatestVersion(hypePath).Return("v0.1.0", nil)
...
func TestGetCurrentHYPEVersion_NotInstalled(t *testing.T) {
	originalHome := os.Getenv("HOME")
	defer os.Setenv("HOME", originalHome)
	...
	_, err := GetCurrentHYPEVersion()
	if err == nil {
		t.Error("Expected error when HYPE not installed, got nil")
	}
}
...
func TestGetLatestHYPEVersion_FallbackParsing(t *testing.T) {
	jsonResponse := `{
		"tag_name": "v0.1.17",
