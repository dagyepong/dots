...
type WindowRulesListResult struct {
	...
	HYPEStatus *windowrules.HYPERulesStatus `json:"hypeStatus,omitempty"`
}
...
func runWindowrulesList(cmd *cobra.Command, args []string) {
	...
	case "niri":
		...
		hypeRulesPath := provider.GetOverridePath()
		hypeRules, _ := provider.LoadHYPERules()

		hypeRuleMap := make(map[int]windowrules.WindowRule)
		for i, dr := range hypeRules {
			hypeRuleMap[i] = dr
		}

		hypeIdx := 0
		for i, r := range allRules {
			if r.Source == hypeRulesPath {
				if dmr, ok := hypeRuleMap[hypeIdx]; ok {
		...
		result.HYPEStatus = parseResult.HYPEStatus
