...
Examples:
  hype color pick                # Pick color, output as hex
  hype color pick --rgb          # Output as RGB
  hype color pick --json         # Output all formats as JSON
  hype color pick --hex -l       # Output hex in lowercase
  hype color pick -a             # Auto-copy result to clipboard`,
	Run: runColorPick,
}
