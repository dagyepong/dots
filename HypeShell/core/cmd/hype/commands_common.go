...
var runCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if v, _ := cmd.Flags().GetString("log-level"); v != "" {
			if err := os.Setenv("HYPE_LOG_LEVEL", v); err != nil {
				log.Fatalf("Failed to set HYPE_LOG_LEVEL: %v", err)
			}
		}
		if v, _ := cmd.Flags().GetString("log-file"); v != "" {
			if err := os.Setenv("HYPE_LOG_FILE", v); err != nil {
				log.Fatalf("Failed to set HYPE_LOG_FILE: %v", err)
			}
		}
...
// Git builds: hype (git) v0.6.2-XXXX
// Stable releases: hype v0.6.2
func formatVersion(version string) string {
	...
	if matches := re.FindStringSubmatch(version); matches != nil {
		return fmt.Sprintf("hype (git) v%s-%s", matches[1], matches[2])
	}
	...
	if matches := re.FindStringSubmatch(version); matches != nil {
		...
		return fmt.Sprintf("hype (git) v%s-%s", baseVersion, matches[1])
	}

	// Stable release format: 0.6.2
	re = regexp.MustCompile(`^([\d.]+)$`)
	if matches := re.FindStringSubmatch(version); matches != nil {
		return fmt.Sprintf("hype v%s", matches[1])
	}
